...
static void QMI8658_doCtrl9Command(enum QMI8658_Ctrl9Command cmd)
{
	QMI8658_write_reg(QMI8658Register_Ctrl9, (unsigned char)cmd);

	const uint32_t start_ms = millis();
	while ((millis() - start_ms) < 20)
	{
		if (QMI8658_readStatus1() & QMI8658_STATUS1_CMD_DONE)
		{
			break;
		}
		delay(1);
...
#else
	gyro[AXIS_X] = (float)(raw_gyro_xyz[AXIS_X] * 0.01745f) / gyro_lsb_div;
...
void QMI8658_enableWakeOnMotion(void)
{
	...
	womCmd[0] = QMI8658Register_Cal1_L;
	womCmd[1] = threshold;
